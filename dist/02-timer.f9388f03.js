...
(0, _flatpickrDefault.default)(date, {
    ...
    onClose (selectedDates) {
        if (selectedDates[0].getTime() <= Date.now()) {
            ...
            btn.disabled = false;
            (0, _notiflixDefault.default).Notify.success("Lets go?");
        }
    }
});
...
function convertMs(ms) {
    ...
    // Remaining minutes
    const minutes = Math.floor(ms % day1 % hour1 / minute);
    // Remaining seconds
    const seconds = Math.floor(ms % day1 % hour1 % minute / second);
    return {
        days,
        hours,
        minutes,
